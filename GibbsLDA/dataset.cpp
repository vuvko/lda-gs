...
int dataset::read_wordmap(string wordmapfile, mapword2id * pword2id) {
    ...
    for (int i = 0; i < nwords; i++) {
        ...
        pword2id->insert(strtok.token(0));
    }
...
int dataset::read_wordmap(string wordmapfile, mapid2word * pid2word) {
    ...
    for (int i = 0; i < nwords; i++) {
        ...
        pid2word->insert(pair<int, string>(strtok.token(0).second, strtok.token(0).first));
    }
...
int dataset::read_trndata(string dfile, string wordmapfile) {
    ...
    for (int i = 0; i < M; i++) {
        ...
        for (int j = 0; j < length; j++) {
            it = word2id.find(strtok.token(j).first);
            if (it == word2id.end()) {
                // word not found, i.e., new word
                pdoc->words[j] = word2id.size();
                word2id.insert(pair<string, int>(strtok.token(j).first, word2id.size()));
            } else {
                pdoc->words[j] = it->second;
            }
            pdoc->word_counts[j] = strtok.token(j).second;
            pdoc->real_length += pdoc->word_counts[j];
        }
    ...
    for (int i = 0; i < M; i++) {
        ...
        for (int j = 0; j < length; j++) {
            ...
            if (it == word2id.end()) {
                ...
                doc.push_back(pair<int, int>(it->second, strtok.token(j).second));
                _doc.push_back(pair<int, int>(_id, strtok.token(j).second));
            }
        }
