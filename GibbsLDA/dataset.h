...
public:
    int * words;
    int * word_counts; // implemented in estimate only
    string rawstr;
    int length;
    int real_length;
    ...
    document(int length) {
        ...
        word_counts = (int *)calloc(length, sizeof(*word_counts));
    }
    
    document(int length, int * words, int * word_counts) {
        this->length = length;
        real_length = 0;
        rawstr = "";
        this->words = new int[length];
        this->word_counts = (int *)calloc(length, sizeof(*word_counts));
        for (int i = 0; i < length; i++) {
            this->words[i] = words[i];
            this->word_counts[i] = word_counts[i];
            real_length += word_counts[i];
        }
    }

    document(int length, int * words, int * word_counts, string rawstr) {
        this->length = length;
        real_length = 0;
        this->rawstr = rawstr;
        this->words = new int[length];
    ...
    document(vector<pair<int, int> > & doc) {
        length = doc.size();
        real_length = 0;
        rawstr = "";
        words = new int[length];
        ...
        for (int i = 0; i < length; i++) {
            words[i] = doc[i].first;
            word_counts[i] = doc[i].second;
            real_length += word_counts[i];
        }
    }

    document(vector<pair<int, int> > & doc, string rawstr) {
        length = doc.size();
        real_length = 0;
        this->rawstr = rawstr;
        words = new int[length];
